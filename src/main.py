...
def on_midi_message(msg, loop):
    ...
    try:
        if b1 == 144:
            ...
            aw = strip.process_button([b2, b3])
            asyncio.run_coroutine_threadsafe(aw, loop)
        elif b1 == 176:
            strip = midi.strips[b2 % 8]
            aw = strip.process_encoder([b2, b3])
            asyncio.run_coroutine_threadsafe(aw, loop)
        else:
            strip = midi.strips[b1 - 224]
            aw = strip.process_fader([b1, b3])
            asyncio.run_coroutine_threadsafe(aw, loop)
    except:
    ...
    while True:
        ...
        for strip in midi.strips:
            if strip.fader_busy and current - strip.fader_delta > FADER_TIMEOUT * 1000000000:
                await asyncio.to_thread(strip.pos_fader)
